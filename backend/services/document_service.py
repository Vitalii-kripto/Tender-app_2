...
from backend.services.download_retry_service import retry_downloaded_file_once
...
class DocumentService:
    ...
    def _iter_docx_blocks(self, container):
    ...
    def _extract_docx_xml_text(self, element) -> str:
        chunks = []
        for node in element.iter():
            ...
            if tag.endswith("}t") and node.text:
                ...
                chunks.append("\n")
        return self._normalize_extracted_text("".join(chunks))

    def _extract_docx_table_rows(self, table) -> List[str]:
        rows = []
        for tr in table._tbl.iterchildren():
            tr_tag = getattr(tr, "tag", None)
            if not isinstance(tr_tag, str) or not tr_tag.endswith("}tr"):
                continue

            row_data = []
            for tc in tr.iterchildren():
                tc_tag = getattr(tc, "tag", None)
                if not isinstance(tc_tag, str) or not tc_tag.endswith("}tc"):
                    continue
                value = self._extract_docx_xml_text(tc)
                row_data.append(value)

            clean_row = [cell for cell in row_data if cell]
            if clean_row:
                rows.append(" | ".join(clean_row))
        return rows

    def _convert_emf_bytes_to_png_bytes(self, image_bytes: bytes) -> bytes:
        import tempfile
        import aspose.imaging as imaging
        import aspose.imaging.imageoptions as imageoptions

        src_file = tempfile.NamedTemporaryFile(delete=False, suffix=".emf")
        dst_file = tempfile.NamedTemporaryFile(delete=False, suffix=".png")
        ...
        try:
            with open(src_path, "wb") as src_file:
                src_file.write(image_bytes)
            with imaging.Image.load(src_path) as image:
                image.save(dst_path, imageoptions.PngOptions())
            if not os.path.exists(dst_path):
                raise RuntimeError("EMF conversion did not produce PNG output")
    ...
    def _load_embedded_image_as_rgb_array(self, image_bytes: bytes, ext: str) -> np.ndarray:
        image_stream = io.BytesIO(image_bytes)
        if ext == ".emf":
            image_stream = io.BytesIO(self._convert_emf_bytes_to_png_bytes(image_bytes))

        with Image.open(image_stream) as image:
            image = image.convert("RGB")
            return np.array(image)

    def _extract_docx_embedded_image_text(self, file_path: str) -> List[Dict[str, Any]]:
        ocr_results: List[Dict[str, Any]] = []
        raster_exts = {".png", ".jpg", ".jpeg", ".bmp", ".tif", ".tiff", ".webp", ".emf"}
        ...
        try:
            with zipfile.ZipFile(file_path) as archive:
                ...
                for media_name in media_names:
                    ...
                    if ext not in raster_exts:
                        ocr_results.append({
                            "name": os.path.basename(media_name),
                            "status": "skipped_unsupported_image_type",
                            "text": "",
                            "error": "",
                        })
                        continue
                    ...
                    try:
                        ...
                        image_array = self._load_embedded_image_as_rgb_array(image_bytes, ext)
                        ocr_text = self._normalize_extracted_text(self._ocr_single_image(image_array))
                        ocr_results.append({
                            "name": os.path.basename(media_name),
                            "status": "success" if ocr_text else "empty",
                            "text": ocr_text,
                            "error": "",
                        })
                        ...
                        logger.warning("DOCX embedded image OCR failed for %s: %s", media_name, exc)
                        ocr_results.append({
                            "name": os.path.basename(media_name),
                            "status": "failed_ocr",
                            "text": "",
            ...
            logger.warning("DOCX media extraction failed for %s: %s", file_path, exc)
    ...
    def _extract_docx_text_safely(self, file_path: str):
        ...
        for block_type, block in self._iter_docx_blocks(doc):
            if block_type == "paragraph":
                value = self._normalize_extracted_text(block.text)
                if value:
                    text_parts.append(value)
                continue

            table_rows = self._extract_docx_table_rows(block)
            if table_rows:
                tables_data.append("\n".join(table_rows))
        ...
        for section in doc.sections:
            ...
            if header:
                for block_type, block in self._iter_docx_blocks(header):
                    if block_type == "paragraph":
                        value = self._normalize_extracted_text(block.text)
                        if value:
                            text_parts.append(value)
            ...
            if footer:
                for block_type, block in self._iter_docx_blocks(footer):
                    if block_type == "paragraph":
                        value = self._normalize_extracted_text(block.text)
                        if value:
                            text_parts.append(value)
        ...
        embedded_image_results = self._extract_docx_embedded_image_text(file_path)
        embedded_image_texts = [
            f"=== OCR IMAGE: {item['name']} ===\n{item['text']}"
            for item in embedded_image_results
            if item.get("status") == "success" and item.get("text")
        ]
        if embedded_image_texts:
            text_parts.extend(embedded_image_texts)
            tables_data.extend(embedded_image_texts)
        ...
        if embedded_image_results:
            pages[0]["embedded_images"] = embedded_image_results
            ...
            try:
                ...
                for attempt in range(3):
                    ...
                    try:
                        ...
                        error_text = str(e)
                        should_retry = "Вызов был отклонен" in error_text or "Call was rejected" in error_text
                        logger.warning(
                            "win32com conversion failed on attempt %s/3 (check if Word is installed): %s",
                            attempt + 1,
                            e,
                        )
                        if doc is not None:
    ...
    def _retry_document_download_once(
        self,
        *,
        file_path: str,
        tender_id: str,
        status: str,
        error_message: str,
        allow_download_retry: bool,
    ) -> Optional[Dict[str, Any]]:
        if not allow_download_retry:
            return None
        ...
        retry_reason = str(error_message or status or "document_retry").strip()
        retry_result = retry_downloaded_file_once(
            file_path,
            reason=retry_reason,
            logger=logger,
        )
        if not retry_result.get("attempted"):
            return None

        if not retry_result.get("succeeded"):
            logger.warning(
                "[DOC_REDOWNLOAD_RETRY_FAILED] tender_id=%s filename='%s' status='%s' error='%s'",
                tender_id,
        ...
        logger.warning(
            "[DOC_REDOWNLOAD_RETRY] tender_id=%s filename='%s' status='%s' reason='%s'",
            tender_id,
            os.path.basename(file_path),
            status,
            retry_reason,
        )
        retried_result = self.extract_document_data(
            file_path,
            use_cache=False,
            tender_id=tender_id,
            allow_download_retry=False,
        )
        retried_result["download_retry_performed"] = True
        retried_result["download_retry_reason"] = retry_reason
    ...
    def extract_document_data(
        ...
        allow_download_retry: bool = True,
    ) -> Dict[str, Any]:
        """
        ...
        if file_size == 0:
            ...
            retried = self._retry_document_download_once(
                file_path=file_path,
                tender_id=tender_id,
                status=result["status"],
                error_message=result["error_message"],
                allow_download_retry=allow_download_retry,
            )
            ...
            if ext == '.doc':
                ...
                if os.path.exists(docx_path):
                    ...
                    if new_path.endswith('.docx'):
                        ...
                        result["text"] = result["extracted_text"]
    ...
    def _perform_ocr(self, file_path: str) -> List[Dict[str, Any]]:
        ...
        render_scale = max(1.0, float(os.environ.get("DOCUMENT_OCR_RENDER_SCALE", "2.0")))
    ...
    def _extract_text_from_doc(self, file_path: str) -> str:
        ...
        if raw_bytes[:32].lstrip().startswith(b"{\\rtf"):
            try:
        ...
        try:
            ...
            if raw_bytes:
                for encoding in ("utf-8", "cp1251", "latin1"):
                    try:
                        decoded_variants.append(raw_bytes.decode(encoding))
                    except UnicodeDecodeError:
                        decoded_variants.append(raw_bytes.decode(encoding, errors="ignore"))
            for raw_content in decoded_variants:
